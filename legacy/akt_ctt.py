...
class AKT_ctt(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks,
        ...
        embed_l = d_model
        if self.n_pid > 0:
            self.difficult_param = nn.Embedding(self.n_pid+1, embed_l) #changed
            self.q_embed_diff = nn.Embedding(self.n_question+1, embed_l)
            self.qa_embed_diff = nn.Embedding(2 * self.n_question + 1, embed_l)
        # n_question+1 ,d_model
        self.q_embed = nn.Embedding(self.n_question+1, embed_l)
        self.diff_embed = nn.Embedding(self.num_diff, embed_l)
        if self.separate_qa:
    ...
    def forward(self, q_data, target, pid_data=None, diff=None):

        qa_data = q_data + (target * self.n_question)

        # Batch First
        q_embed_data = self.q_embed(q_data)  # BS, seqlen,  d_model# c_ct
        diff_embed = self.diff_embed(diff)
        ...
        if self.n_pid > 0:
            ...
            q_embed_data = q_embed_data + pid_embed_data + diff_embed   # uq *d_ct + c_ct

            qa_embed_diff_data = self.qa_embed_diff(qa_data)  # f_(ct,rt) or #h_rt

            if self.separate_qa:
                qa_embed_data = qa_embed_data + pid_embed_data + diff_embed  # uq* f_(ct,rt) + e_(ct,rt)
            else:
                qa_embed_data = qa_embed_data + pid_embed_data + diff_embed  # + uq *(h_rt+d_ct)
...
def attention(q, k, v, d_k, mask, dropout, zero_pad, gamma=None):
    ...
    # x1 = [[0, 1, .., 199], [0, 1, ~~, 199]...[0, 1, ~~, 199]]
    # |x1| = (200, 200)
    x2 = x1.transpose(0, 1).contiguous()
    # x2 = [[0, 0, .., 0], [1, 1, ..., 1], ... [199, 199, ..., 199]]
    # |x2| = (200, 200)

    with torch.no_grad():
        scores_ = scores.masked_fill(mask == 0, -1e32)
        # |scores_| = (24, 8, 200, 200)
        scores_ = F.softmax(scores_, dim=-1)  # BS,8,seqlen,seqlen
        # |scores_| = (24, 8, 200, 200)
        scores_ = scores_ * mask.float().to(device)
        # |scores_| = (24, 8, 200, 200)
        # => scores는 softmax를 거쳤으므로, 확률값임

        distcum_scores = torch.cumsum(scores_, dim=-1)  # bs, 8, sl, sl
        # |distcum_scores| = (24, 8, 200, 200)
        disttotal_scores = torch.sum(
            scores_, dim=-1, keepdim=True)  # bs, 8, sl, 1
        # |disttotal_scores| = (24, 8, 200, 1), 1 아니면 0
        position_effect = torch.abs(
            x1-x2)[None, None, :, :].type(torch.FloatTensor).to(device)  # 1, 1, seqlen, seqlen
        # |torch.abs(x1-x2)| = (200, 200)
        # |position_effect| = (1, 1, 200, 200)
