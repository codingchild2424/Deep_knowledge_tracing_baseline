...
class DKT_c_rasch_trainer():
    ...
    def _train(self, train_loader, metric_name):
        ...
        for idx, data in enumerate(tqdm(train_loader)):
            self.model.train()
            q_seqs, r_seqs, qshft_seqs, rshft_seqs, pid_seqs, mask_seqs = data #collate에 정의된 데이터가 나옴
            q_seqs = q_seqs.to(self.device) #|q_seqs| = (bs, sq) 
            r_seqs = r_seqs.to(self.device) #|r_seqs| = (bs, sq)
            pid_seqs = pid_seqs.to(self.device)
            qshft_seqs = qshft_seqs.to(self.device) #|qshft_seqs| = (bs, sq) 
            rshft_seqs = rshft_seqs.to(self.device) #|rshft_seqs| = (bs, sq) 
            mask_seqs = mask_seqs.to(self.device) #|mask_seqs| = (bs, sq) 

            y_hat = self.model( q_seqs.long(), r_seqs.long(), pid_seqs.long() ) #|y_hat| = (bs, sq, self.num_q) -> tensor([[[0.6938, 0.7605, ..., 0.7821], [0.8366, 0.6598,  ..., 0.8514],..)
    ...
    def _validate(self, valid_loader, metric_name):
        ...
        with torch.no_grad():
            for data in tqdm(valid_loader):
                ...
                y_hat = self.model( q_seqs.long(), r_seqs.long(), pid_seqs.long() )
