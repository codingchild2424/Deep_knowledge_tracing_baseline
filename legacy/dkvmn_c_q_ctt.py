...
class DKVMN_c_q_ctt(Module):
    ...
    '''
    def __init__(self, num_q, num_pid, num_diff, dim_s, size_m):
        super().__init__()
        self.num_q = num_q
        ...
        self.diff_emb_layer = Embedding(self.num_diff, self.dim_s)
    ...
    def forward(self, q, r, pid, diff):
        ...
        k = self.k_emb_layer(q) + self.pid_emb_layer(pid) + self.diff_emb_layer(diff)

        #|v| = (bs, 문항의 정오답에 따른 인덱스값, dim_s)
        v = self.v_emb_layer(x) + self.pid_emb_layer(pid) + self.diff_emb_layer(diff)
