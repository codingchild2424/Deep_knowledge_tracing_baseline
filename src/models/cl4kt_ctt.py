...
class CL4KT_ctt(Module):
    def __init__(self, num_skills, num_questions, seq_len, **kwargs):
        ...
        self.pid_embed = Embedding(
            self.num_questions, self.hidden_size, padding_idx=0
        )

        self.diff_embed = Embedding(
            self.num_diff, self.hidden_size, padding_idx=0
        )
    ...
    def forward(self, q_seqs, r_seqs, pid_seqs, diff_seqs, mask_seqs):
        if self.training:
            q_i = q_seqs  
            q_j = q_seqs
            q = q_seqs  # augmented q_i, augmented q_j and original q
            pid_i = pid_seqs 
            pid_j = pid_seqs 
            pid = pid_seqs
            r_i = r_seqs
            ...
            r = r_seqs
            neg_r = r_seqs
            attention_mask_i = mask_seqs
            attention_mask_j = mask_seqs
            attention_mask = mask_seqs

            pid_i_embed = self.pid_embed(pid)
            pid_j_embed = self.pid_embed(pid)

            diff_embed = self.diff_embed(diff_seqs)

            # qustion emb
            ques_i_embed = self.question_embed(q_i) + pid_i_embed + diff_embed
            ques_j_embed = self.question_embed(q_j) + pid_j_embed + diff_embed

            # interaction emb
            inter_i_embed = self.get_interaction_embed(q_i, r_i) + pid_i_embed + diff_embed
            inter_j_embed = self.get_interaction_embed(q_j, r_j) + pid_j_embed + diff_embed
            ...
            q = q_seqs  # augmented q_i, augmented q_j and original q
            r = r_seqs  # augmented r_i, augmented r_j and original r
