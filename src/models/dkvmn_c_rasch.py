...
class DKVMN_c_rasch(Module):
    ...
    def __init__(self, num_q, num_pid, dim_s, size_m):
        ...
        self.pid_emb_layer = Embedding(self.num_pid, 1)
    ...
    def forward(self, q, r, pid):
        ...
        k = self.k_emb_layer(q) + self.pid_emb_layer(pid) * self.k_emb_layer(q)

        #|v| = (bs, 문항의 정오답에 따른 인덱스값, dim_s)
        v = self.v_emb_layer(x) + self.pid_emb_layer(pid) * self.v_emb_layer(x)
