...
class SAKT_c_rasch(Module):
    def __init__(self, num_q, num_pid, n, d, num_attn_heads, device, dropout=.2): #device를 추가함
        super().__init__()
        ...
        self.emb_pid = Embedding(self.num_pid, 1)
    ...
    def forward(self, q, r, qry, pid):
        ...
        M = self.M(x) + self.emb_pid(pid) * self.M(x)

        E = self.E(qry) + self.emb_pid(pid) * self.E(qry)
