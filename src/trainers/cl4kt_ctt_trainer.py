...
class cl4kt_ctt_trainer():
    ...
    def _train(self, train_loader, metric_name):
        ...
        for idx, data in enumerate(tqdm(train_loader)):
            ...
            y_hat, true = self.model( q_seqs.long(), r_seqs.long(), pid_seqs.long(), diff_seqs.long(), mask_seqs.long() ) #|y_hat| = (bs, sq, self.num_q) -> tensor([[[0.6938, 0.7605, ..., 0.7821], [0.8366, 0.6598,  ..., 0.8514],..)

            y_hat = torch.masked_select(y_hat, mask_seqs[:, :-1]) #|y_hat| = () -> tensor([0.7782, 0.8887, 0.7638,  ..., 0.8772, 0.8706, 0.8831])
            #=> mask를 활용해서 각 sq 중 실제로 문제를 푼 경우의 확률값만 추출

            correct = torch.masked_select(true, mask_seqs[:, :-1]) #|correct| = () -> tensor([0., 1., 1.,  ..., 1., 1., 1.])
